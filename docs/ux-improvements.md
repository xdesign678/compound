...
### 1. 列表虚拟化 + 干掉冗余 `toArray()` ✅ 已完成（部分）

> 已完成：conceptCountBySource 从 N+1 改为单次遍历建 Map；WikiView 移除 allConceptsForReview 全量查询；unreviewedCount 改用 getUnreviewedCountFromDb；LibraryView 已有 visibleCount 分页。
> 未完成：RecapView/HealthView 的 toArray() 仍保留（前者为功能需要，后者为一次性 lint 操作）。
...
> 全局任务浮窗（TaskCenter 组件），跟踪 ingest/lint/repair/categorize/query 状态、进度、失败重试。摄入操作从阻塞 toast 改为后台任务。store 新增 TaskItem 类型。
...
> navigator.onLine 监听 + 顶部离线横幅 + 写操作 disable + /offline 页 SW 兜底 + api-client 断网区分
...
> Tab hover/focus 预加载对应视图 chunk，消除首切顿挫。
...
> Lora/Noto Serif SC display 改为 optional；删除未引用大图 icon-ai.png/icon-ai2.png (~1.7MB)；修复 mounted→ready 双骨架闪烁；SW 缓存注释。
...
> NoteEditor 重构：标题从内容推断改为独立输入框；草稿从单 slot 改为 compound*note_draft*{id} 多 slot；修复 body ? trimmed : trimmed 逻辑错误。未引入 tiptap/lexical（高风险，留后续）。
...
### 9. 空态 / Onboarding 引导 ✅ 已完成

> 新建 OnboardingCard 组件（粘贴正文/Obsidian导入/GitHub同步三选一）；seed 自动标记 compound_is_sample；DataTab 新增「清除示例数据」操作。
...
> LibraryView: 滚动恢复增加锚定概念 ID (data-concept-id)，避免 dexie 二次 update 跳动；AskView: 自动滚底改为仅当用户在底部 50px 内才跟随。
...
> 新增 useSelectionPopover hook，单实例管理，桌面双栏不重复绑定。
...
| 5         | 任务中心 (#2)                | 中    | 重塑长任务体验         | ✅          |
| 6         | 列表虚拟化 (#1)              | 中-高 | 1k 概念后必须          | ✅ (部分)   |
| 7         | 编辑器替换 (#7)              | 高    | 长期债务               | ✅ (改进版) |
