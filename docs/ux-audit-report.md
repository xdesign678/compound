...
> **审计团队**: Agent A (状态反馈) + Agent B (导航心智) + Agent C (视觉/CSS) + Agent D (错误恢复) + Agent E (移动端 PWA)
...
## 补充发现 (Agent D 错误恢复 + Agent E 移动端 PWA)

以下为 Agent D 和 Agent E 提交的增量发现（已去重，不含前文已列出的问题）。

### 🟠 补充 Major

#### SM-01 | 概念详情无编辑/删除操作 (H3 用户控制)

- **页面**: 概念详情页 | Desktop + Mobile
- **现状**: 概念详情中完全没有编辑、删除、归档等操作按钮。AI 生成的概念用户无法修改或删除
- **修复**: 至少提供"标记为错误"/"请求修正"/"删除"操作

#### SM-02 | 桌面分栏详情面板无关闭按钮 (H3)

- **页面**: Desktop Wiki Tab → 点击概念 → 右侧面板
- **现状**: 概念详情以分栏显示，没有 X 按钮或关闭图标。只能通过 Escape 或点其他概念切换
- **修复**: 在分栏面板顶部加"收起面板" ✕ 按钮

#### SM-03 | 问答失败缺少"重试"按钮 (H9)

- **页面**: Ask Tab → API 失败后
- **现状**: 显示"问答暂时没成功"+"新对话"按钮，但没有"重新提问"按钮
- **修复**: 在失败消息旁增加"重新提问"按钮，复用上一条问题

#### SM-04 | 没有自定义 404 页面 (H9 + H10)

- **页面**: 访问不存在的路由如 `/concept/xxx`
- **现状**: 显示 Next.js 默认英文 "404: This page could not be found." 无品牌样式
- **修复**: 创建自定义 `not-found.tsx`，中文提示 + 返回首页 + 搜索建议

#### SM-05 | Ask 页暴露原始 JSON 错误给用户 (H9)

- **页面**: 问答 Tab → API 调用失败
- **现状**: 显示 `concept detail failed (401): {"error":"Unauthorized","requestId":"..."}`
- **修复**: 技术信息折叠/隐藏，仅显示友好文案 + 可选"查看详情"展开

#### SM-06 | 一级分类横向滚动无视觉暗示 (H4 移动端)

- **页面**: Wiki Tab | Mobile 390×844
- **现状**: 分类卡片横向溢出，右侧截断显示"软件架"，但无渐变遮罩或箭头暗示可滑动
- **修复**: 添加右侧 gradient fade-out `::after` 遮罩（桌面端已有类似实现，需扩展到移动端）

#### SM-07 | Recap 返回按钮触摸区域偏小 36px (H3 移动端)

- **页面**: `/recap` | Mobile
- **现状**: `.recap-back-btn` 36×36px，低于 Apple HIG 44px 推荐值，且位于左上角拇指难达区
- **修复**: 提升到 44px；考虑让 SwipeBack 手势也在 recap 页面生效

### 🟡 补充 Minor

#### sm-01 | "+" 按钮视觉触摸区域 40px (H7 移动端)

- **页面**: 底部 Tab 中间 "+" 按钮 | Mobile
- **现状**: 外层 70×65px 足够，但视觉黑色圆圈仅 40px，用户感知偏小
- **修复**: 视觉尺寸从 40px 提升到 44px

#### sm-02 | 活动流 filter-chip 触摸区域偏小 36px (H7 移动端)

- **页面**: Activity Tab 筛选标签 | Mobile
- **现状**: `min-height: 36px` 低于 44px，密集排列容易误触
- **修复**: 移动端添加 `min-height: 44px; padding: 8px 14px`

#### sm-03 | 搜索框聚焦时缺少"取消"按钮 (H3 移动端)

- **页面**: Wiki Tab 搜索框 | Mobile
- **现状**: iOS 键盘弹出后无"取消"按钮让用户快速退出搜索
- **修复**: 聚焦时右侧动画展示"取消"文字按钮（iOS 标准模式）

#### sm-04 | 暗色模式下底部 Tab 栏分隔不明显 (H4 移动端)

- **页面**: 所有 Tab 暗色模式 | Mobile
- **现状**: `border-top` 对比度不足，Tab 栏与内容区分界不清
- **修复**: 暗色模式增加微妙上方阴影或加深 border-color

#### sm-05 | 搜索无结果缺少建议 (H10)

- **页面**: Wiki Tab 搜索不存在关键词
- **现状**: 显示"没有匹配的概念"+"清空筛选"，缺少相似词建议
- **修复**: 添加"试试换个关键词"引导提示

### 🔵 补充 Enhancement

#### SE-01 | SwipeBack 手势扩展到 recap 页面

- 当前仅在 detail 状态激活，recap 页面用户只能点 36px 返回按钮

#### SE-02 | 底部 Tab 添加触觉反馈

- Tab 切换有视觉 scale 反馈但无 haptic 震动，添加 `hapticLight()` 提升原生感

#### SE-03 | 概念详情页添加阅读进度指示器

- 长文无阅读进度，可在顶部添加细线进度条

#### SE-04 | PWA 安装引导横幅

- 有 manifest + SW 但无主动安装引导，建议首次访问展示非侵入式安装 toast

---

## 更新后的发现总计

| 严重程度       | 原始   | Agent D+E 增量 | 总计   |
| -------------- | ------ | -------------- | ------ |
| 🔴 Critical    | 7      | 0              | **7**  |
| 🟠 Major       | 12     | +7             | **19** |
| 🟡 Minor       | 13     | +5             | **18** |
...
4. **分类筛选** — 带计数的一级分类按钮，渐进式筛选直观
5. **日期本地化** — "1 小时前"、"昨天" 相对时间格式良好

### 离线与系统

6. **离线架构** — OfflineBanner + Toast 暂停通知 + TaskCenter paused-offline 状态，完整的离线体验
7. **错误恢复页** — 重试 + 回首页 + 复制 Sentry ID 三个操作选项，设计周到
8. **性能优化** — 动态 import + Tab 预加载 + critical CSS + SW 缓存策略
9. **TaskCenter** — 5 种任务状态 + 离线暂停 + 自动重试 + 浮动按钮计数

### 移动端 PWA

10. **底部 Tab 触摸目标充足** — 每个 tab 75×65px，远超 44px 最小值
11. **Safe Area 处理完善** — `env(safe-area-inset-bottom)` 在 tabbar、modal、输入条等关键位置均被引用
12. **键盘适配优秀** — `ViewportObserver` 使用 `visualViewport` API 实时跟踪软键盘高度
13. **输入框 16px 防缩放** — 避免 iOS Safari 自动缩放
14. **SwipeBack 手势** — 有边缘滑动返回+触觉反馈+进度指示器
15. **Modal 为 Bottom Sheet** — 从底部滑出，带 handle 把手，符合移动端范式
16. **概念卡片 `:active` 反馈** — `transform: scale(0.992)` 模拟按压感
17. **Scroll snap** — 分类卡片使用 `scroll-snap-type: inline proximity` 提升滚动体验

### 无障碍

18. **ARIA 标签完善** — 焦点管理、键盘导航、`aria-live`、`aria-busy`
19. **Loading skeleton 有 `role="status"` + `aria-busy="true"`**
20. **`prefers-reduced-motion` 支持** — 全局禁用动画
