...
test('repair-worker: missing-link adds bidirectional related ids', { concurrency: false }, async (t) => {
...
test('repair-worker: duplicate merge deletes secondary and rewires references', { concurrency: false }, async (t) => {
  const env = setupTempDb();
  t.after(env.cleanup);
  ...
  await withMockFetch(mockFetch, async () => {
    const { createRepairRun, startRepairWorker, getRepairRunStatus } = await import('./repair-worker');
    const { repo } = await import('./server-db');
...
test('repair-worker: merge falls back to mechanical join when LLM fails', { concurrency: false }, async (t) => {
  const env = setupTempDb();
  t.after(env.cleanup);
...
test('repair-worker: createRepairRun caps jobs and reports dropped count', { concurrency: false }, async (t) => {
  const env = setupTempDb();
  t.after(env.cleanup);
