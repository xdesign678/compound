...
export const SEED_CONCEPTS: Concept[] = [
  {
    id: 'c-seed-1',
    title: 'LLM Wiki 模式',
    summary: '由 LLM 维护的持久化 Markdown Wiki,替代每次从零检索的 RAG。核心是把知识预先综合而不是临时拼接。',
...
Wiki 有双重身份:**对人类是可浏览的知识界面,对 AI 是预先综合的知识层**——同一个产物,两种受众。`,
  ...
  },
  {
    id: 'c-seed-2',
    title: 'RAG 的结构性缺陷',
    summary: '"LLM 在每次提问时都是从零重新发现知识,没有任何积累。" 检索即用即弃,跨文档综合能力缺失。',
...
**第三层 Schema**:配置文件,定义 Wiki 的结构规范、命名约定、工作流。类似给知识库专用的 CLAUDE.md。`,
    ...
    version: 1,
    categories: [{ primary: '人工智能', secondary: '知识系统' }, { primary: '软件架构', secondary: '系统设计' }],
...
**Lint**:定期体检,找矛盾声明、孤立页面、缺失链接。LLM 最擅长、人类最讨厌的工作。`,
  ...
  },
  {
    ...
    summary: '"维护知识库中最累的不是阅读或思考——而是记账整理。" LLM 正好反过来:读不快想不深,但并行维护不累。',
...
LLM 恰好反过来。读不快、想不深,但同时更新十个文件不喊累。当 bookkeeping 的成本趋近于零,个人知识系统才第一次真正可行。`,
    ...
    version: 1,
    categories: [{ primary: '人工智能', secondary: '知识系统' }, { primary: '认知科学', secondary: '认知负荷' }],
...
Memex 80 年来未被真正兑现的原因是[关联维护成本太高](concept:c-seed-5)。LLM Wiki 被 Karpathy 视为这个想法的当代兑现。`,
  ...
  },
  {
    ...
    summary: '从 prompt engineering 演进到的新学科:工程化地构建高质量上下文,而不是琢磨 prompt 措辞。',
...
[LLM Wiki](concept:c-seed-1) 是 context engineering 的最完整具象:你不是让 AI 每次面对原始文本,而是为 AI 准备一个结构化、可维护、持续演化的知识操作系统。`,
  ...
  },
  {
    ...
    summary: 'Luhmann 的卡片笔记法。原子化笔记 + 密集链接。与 LLM Wiki 同构,但需要人类执行,因此大多数人失败。',
