...
exports.buildSyncStageItems = buildSyncStageItems;
exports.getCurrentFileDisplay = getCurrentFileDisplay;
exports.getSyncStatusCopy = getSyncStatusCopy;
function buildSyncStageItems(input) {
    const active = inferActiveStage(input);
    const orderedIds = ['scan', 'plan', 'process', 'pull'];
...
function getCurrentFileDisplay(current) {
    if (!current)
        return { counter: null, path: null };
...
function getSyncStatusCopy(input) {
    if (input.phase === 'idle') {
        return {
            eyebrow: '等待开始',
            title: '准备从 GitHub 同步',
...
function inferActiveStage(input) {
    if (input.phase === 'idle' || input.phase === 'starting')
        return 'scan';
    if (input.phase === 'done' && input.pulling)
        return 'pull';
    if (input.phase === 'done' && !input.pulling)
        return 'pull';
    const current = input.job?.current || '';
    if (current.includes('扫描 GitHub 仓库'))
        return 'scan';
    if (current.includes('比对本地差异'))
        return 'plan';
    if (/^\[\d+\/\d+\]\s+.+/.test(current))
        return 'process';
    if ((input.job?.done || 0) + (input.job?.failed || 0) > 0)
        return 'process';
    if ((input.job?.total || 0) > 0)
        return 'plan';
    return 'scan';
}
